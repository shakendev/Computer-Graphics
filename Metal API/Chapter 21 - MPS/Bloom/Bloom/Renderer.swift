...
extension Renderer: MTKViewDelegate {
  func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
    ...
    outputTexture = createTexture(pixelFormat: view.colorPixelFormat,
                                  size: size)
    finalTexture = createTexture(pixelFormat: view.colorPixelFormat,
                                 size: size)
  }
  ...
  func draw(in view: MTKView) {
    ...
    let brightness = MPSImageThresholdToZero(device: Renderer.device,
                                             thresholdValue: 0.2,
    ...
    blitEncoder.copy(from: finalTexture, sourceSlice: 0, sourceLevel: 0,
